...
const DEFAULT_COURSES: Course[] = [
  { id: 'generative-ai', icon: '🤖', tag: 'AI & ML', tagColor: 'rgba(192,132,252,.75)', iconBg: 'rgba(192,132,252,.08)', borderColor: 'rgba(192,132,252,.12)', title: 'Generative AI', description: 'Prompt engineering, LLMs, RAG pipelines, and building AI-powered apps from scratch.', price1: 399, price3: 699, price6: 999, published: true, parts: [], createdAt: '', updatedAt: '' },
  { id: 'web-development', icon: '🌐', tag: 'Dev', tagColor: 'rgba(52,211,153,.75)', iconBg: 'rgba(52,211,153,.06)', borderColor: 'rgba(52,211,153,.1)', title: 'Web Development', description: 'HTML, CSS, JavaScript, React & Node.js. Build and deploy full-stack web apps.', price1: 399, price3: 699, price6: 999, published: true, parts: [], createdAt: '', updatedAt: '' },
  { id: 'digital-marketing', icon: '📈', tag: 'Marketing', tagColor: 'rgba(251,191,36,.75)', iconBg: 'rgba(251,191,36,.06)', borderColor: 'rgba(251,191,36,.1)', title: 'Digital Marketing', description: 'SEO, social media, content strategy, and analytics to grow brands online.', price1: 399, price3: 699, price6: 999, published: true, parts: [], createdAt: '', updatedAt: '' },
  { id: 'java-programming', icon: '☕', tag: 'Backend', tagColor: 'rgba(96,165,250,.75)', iconBg: 'rgba(96,165,250,.06)', borderColor: 'rgba(96,165,250,.1)', title: 'Java Programming', description: 'Core Java, OOP, data structures, Spring Boot basics and backend development.', price1: 399, price3: 699, price6: 999, published: true, parts: [], createdAt: '', updatedAt: '' },
  { id: 'cpp-fundamentals', icon: '⚡', tag: 'Systems', tagColor: 'rgba(248,113,113,.75)', iconBg: 'rgba(248,113,113,.06)', borderColor: 'rgba(248,113,113,.1)', title: 'C++ Fundamentals', description: 'Pointers, memory, STL, algorithms and competitive programming preparation.', price1: 399, price3: 699, price6: 999, published: true, parts: [], createdAt: '', updatedAt: '' },
]
...
export default function Home() {
  ...
  useEffect(() => {
    fetch('/api/courses')
      .then(r => r.json())
      .then(d => {
        const published = (d as Course[]).filter(c => c.published)
        if (published.length > 0) setCourses(published)
      })
      .catch(() => {})
  }, [])
  ...
  function getPrice(c: Course, d: 1 | 3 | 6) {
    return d === 1 ? c.price1 : d === 3 ? c.price3 : c.price6
  }
    ...
    try {
      const res = await fetch('/api/enroll', {
        ...
        body: JSON.stringify({
          ...
          amount: getPrice(selectedCourse, selectedDuration),
          isDummy: true,
          name: 'Test User',
          email: 'test@test.com',
        }),
      ...
      try { localStorage.setItem(`elevate_enrollment_${selectedCourse.id}`, data.enrollmentId) } catch {}
      closeModal()
  ...
  const selectedPrice = selectedCourse ? getPrice(selectedCourse, selectedDuration) : 0
    ...
    <>
      ...
      <section id="courses" className="courses-bg" style={{ padding: '80px 24px' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto' }}>
          ...
          <div className="cgrid">
            {courses.map(c => (
              <div key={c.id} className="ccard lg" onClick={() => openModal(c)}
                style={{ borderColor: c.borderColor }}>
                <div className="ccard-in">
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div className="cico" style={{ background: c.iconBg, border: `1px solid ${c.borderColor}` }}>{c.icon}</div>
                    <span className="ctag" style={{ color: c.tagColor, background: c.iconBg, border: `1px solid ${c.borderColor}` }}>{c.tag}</span>
                  </div>
                  <div><div className="ctitle">{c.title}</div><div className="cdesc">{c.description}</div></div>
                  <div className="cmeta">
      ...
      <div className="footer-wrap">
        <footer>
          <div className="ft">
            ...
            <div className="fc"><h4>Courses</h4><a href="#">Generative AI</a><a href="#">Web Development</a><a href="#">Digital Marketing</a><a href="#">Java Programming</a><a href="#">C++ Fundamentals</a></div>
      ...
      {modalOpen && selectedCourse && (
        <div className="modal-ov on" onClick={e => { if (e.target === e.currentTarget) closeModal() }}>
          <div className="mbox lg">
            <button className="mcl" onClick={closeModal}>×</button>
            <div className="mt">Enroll Now</div>
            <div className="mc">{selectedCourse.title}</div>
            <div className="ml">Choose Duration</div>
            <div className="dopts">
              {([1, 3, 6] as const).map(d => (
                <div key={d} className={`dopt${selectedDuration === d ? ' sel' : ''}`} onClick={() => setSelectedDuration(d)}>
                  <div className="dm">{d}</div>
                  <div className="dl">{d === 1 ? 'Month' : 'Months'}</div>
                  <div className="dp2">₹{getPrice(selectedCourse, d)}</div>
                </div>
              ))}
            </div>
            <div className="mtot">
              <div className="mtl">Total</div>
              <div className="mtp">₹{selectedPrice.toLocaleString('en-IN')}</div>
            </div>
            <button className="menroll">Proceed to Payment →</button>
            <div style={{ margin: '12px 0 0', textAlign: 'center' }}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 8 }}>— or —</div>
              <button
                onClick={handleTestAccess}
                disabled={enrolling}
                style={{ width: '100%', borderRadius: 100, padding: '11px', border: '1px dashed rgba(251,146,60,.4)', background: 'rgba(251,146,60,.06)', color: '#fb923c', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.2s', opacity: enrolling ? 0.6 : 1 }}>
                {enrolling ? 'Opening...' : '🧪 Test Access (Demo — skip payment)'}
              </button>
            </div>
          </div>
        </div>
